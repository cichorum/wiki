---
created: 2026-01-29T19:12:28-05:00
modified: 2026-01-30T15:24:25-05:00
title: Meet The Team
---

<!--
HTML Team Grid Implemented Via:
  - Typescript Component: quartz/components/TeamMembersGrid.tsx
  - Import/Export: quartz/components/index.ts
  - ConditionalRender of Component.TeamMembersGrid: ~/quartz.layout.ts
-->


## The Client

**Name:** John B. Smith (UNC Computer Science, emmeritus)
**Email:** jbs@cs.unc.edu
**Link:** [https://chicorylane.com/](https://chicorylane.com/)


## Team Rules

### Team Behaviors
- Placeholder expectation for respectful communication  
- Placeholder expectation for meeting attendance  
- Placeholder expectation for collaboration and accountability  

### Coding Practices
- Placeholder guideline for code readability  
- Placeholder guideline for documentation  
- Placeholder guideline for testing and reviews  

### Git Practices
- Placeholder branch naming convention  
- Placeholder commit message expectations  
- Placeholder pull request and review process